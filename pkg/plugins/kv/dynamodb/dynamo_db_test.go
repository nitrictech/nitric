...
import (
	"fmt"
	"github.com/nitric-dev/membrane/pkg/plugins/kv/dynamodb"
...
func (m *mockDynamoDBClient) PutItem(input *dynamodb.PutItemInput) (*dynamodb.PutItemOutput, error) {
	tableName := input.TableName
	item := input.Item
	mapValue := dynamodb_service.NitricKVDocument{}
...
func (m *mockDynamoDBClient) GetItem(input *dynamodb.GetItemInput) (*dynamodb.GetItemOutput, error) {
	...
	if item, ok := m.store[*tableName][*key]; ok {
		attValue, _ := dynamodbattribute.MarshalMap(dynamodb_service.NitricKVDocument{
			Key:   *key,
			Value: item,
		})
...
var _ = Describe("DynamoDb", func() {
	Context("Document Creation", func() {
		When("The dynamo-db client is operational", func() {
			...
			// Inject the mock
			myMockClient, _ := dynamodb_service.NewWithClient(mockSvc)
