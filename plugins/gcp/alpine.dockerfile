...
COPY --from=build ./lib/documents/firestore.so /plugins/documents.so
COPY --from=build ./lib/eventing/pubsub.so /plugins/eventing.so
COPY --from=build ./lib/storage/storage.so /plugins/storage.so
