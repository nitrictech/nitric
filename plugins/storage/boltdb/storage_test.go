...
var _ = Describe("Storage", func() {
	storagePlugin, err := storage_plugin.New()
	if err != nil {
		panic(err)
	}

	AfterEach(func() {
		err := os.RemoveAll(storage_plugin.DEFAULT_DIR)
		if err != nil {
			panic(err)
		}

		_, err = os.Stat(storage_plugin.DEFAULT_DIR)
		if os.IsNotExist(err) {
			// Make diretory if not present
			err := os.Mkdir(storage_plugin.DEFAULT_DIR, 0777)
			if err != nil {
				panic(err)
			}
		}
	...
	AfterSuite(func() {
		err := os.RemoveAll(storage_plugin.DEFAULT_DIR)
		if err == nil {
			os.Remove(storage_plugin.DEFAULT_DIR)
			os.Remove("nitric/")
		}
	})
