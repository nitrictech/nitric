...
In `core/pkg/proto/resources/v1/resources.proto`, add your new resource type to the `ResourceType` enum:

```protobuf title:core/pkg/proto/resources/v1/resources.proto
enum ResourceType {
  // ... existing types ...
...
If your resource needs deployment-specific configuration, update `core/pkg/proto/deployments/v1/deployments.proto`:

```protobuf title:core/pkg/proto/deployments/v1/deployments.proto
...
Run the proto generation scripts to create Go code from your proto definitions.
...
If your resource needs runtime APIs (like storage read/write, topic publish, etc.), implement the corresponding gRPC service in your provider.
