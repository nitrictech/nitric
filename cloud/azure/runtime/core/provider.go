...
func (p *azProviderImpl) GetResources(ctx context.Context, r AzResource) (map[string]AzGenericResource, error) {
	...
	if _, ok := p.cache[r]; !ok {
		...
		for results.NotDone() {
			...
			if tagV, ok := resource.Tags[fmt.Sprintf("x-nitric-stack-%s", p.stack)]; ok && tagV != nil {
				// Add it to the cache
...
func New() (*azProviderImpl, error) {
	...
	stack := os.Getenv(NITRIC_STACK)
	if stack == "" {
		return nil, fmt.Errorf("envvar %s is not set", NITRIC_STACK)
	}
