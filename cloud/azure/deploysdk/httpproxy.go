...
func (p *NitricAzurePulumiProvider) Http(ctx *pulumi.Context, parent pulumi.Resource, name string, http *deploymentspb.Http) error {
	...
	mgmtService, err := apimanagement.NewApiManagementService(ctx, utils.ResourceName(ctx, name, utils.ApiManagementProxyRT), &apimanagement.ApiManagementServiceArgs{
	...
	proxyApi, err := apimanagement.NewApi(ctx, utils.ResourceName(ctx, name, utils.ApiHttpProxyRT), &apimanagement.ApiArgs{
